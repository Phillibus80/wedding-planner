...
if (
    !isset(Flight::request()->data->image_path)
    || Flight::request()->data->image_path == ""
) {
    Flight::response()->header("Content-Type", "application/json");
    Flight::response()->status(400);
    Flight::response()->write(json_encode(array(
        'status' => 400,
        'message' => 'All fields are required: image_path.'
    )));
    ...
    try {
        ...
        if (isset($decodedToken->exp) && ($decodedToken->exp > time())) {
            if (isset($decodedToken->user->username) && isset($decodedToken->user->signedIn) && $decodedToken->user->signedIn) {
                if (file_exists(Flight::request()->data->image_path)) {
                    if (unlink(Flight::request()->data->image_path)) {
